...
function renderRoute(permisos: string[]) {
  ...
  render(<AuthContext.Provider value={{ usuario, isAuthenticated: true, isLoading: false, login: async () => {}, logout: () => {}, refreshSession: async () => true }}><MemoryRouter initialEntries={['/reservas/nueva']}><Routes><Route path="/main" element={<p>Inicio seguro</p>} /><Route element={<ProtectedRoute permissions={['SOLICITUD_CREAR']} />}><Route path="/reservas/nueva" element={<p>Formulario protegido</p>} /></Route></Routes></MemoryRouter></AuthContext.Provider>)
}
