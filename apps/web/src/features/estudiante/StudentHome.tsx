...
export function StudentHome() {
  ...
  useEffect(()=>{let activo=true;Promise.all([obtenerMiContextoAcademico(),obtenerCarreras(),obtenerPeriodos(),obtenerMaterias(),obtenerLaboratorios(),listarSesionesAbiertas(),obtenerPisos()]).then(async ([ctx,carreras,periodos,materias,labs,sesiones,pisos])=>{
    const horario=await obtenerMiHorario(ctx.periodoId);const ids=[...new Set(horario.map(x=>x.docenteId).filter((id):id is string=>Boolean(id)))];const resumenes=await Promise.all(ids.map(id=>obtenerDocenteResumen(id).catch(()=>null)));const pisosPorId=new Map((pisos??[]).map(x=>[x.id,x.numero]));if(activo)setData({carrera:carreras.find(x=>x.id===ctx.carreraId)?.nombre??'Carrera institucional',nivel:ctx.nivel,ciclo:periodos.find(x=>x.id===ctx.periodoId)?.nombre??'Ciclo académico',horario,sesiones,materias:new Map(materias.map(x=>[x.id,x.nombre])),labs:new Map(labs.map(x=>[x.id,x.codigo])),ubicaciones:new Map(labs.map(x=>[x.id,pisosPorId.get(x.pisoId)??0])),docentes:new Map(resumenes.filter(x=>x!==null).map(x=>[x.id,`${x.nombres} ${x.apellidos}`]))})
