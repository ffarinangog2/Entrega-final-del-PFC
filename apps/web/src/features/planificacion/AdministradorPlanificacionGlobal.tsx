...
export function AdministradorPlanificacionGlobal() {
  ...
  return <DashboardLayout breadcrumb="Supervisión de planificación"><section className="operations">
    <header><div><h1>Planificación académica global</h1><p>Planes agregados por carrera y período académico.</p></div></header>
    ...
    {!cargando && !error && <><div className="operations__form">
      ...
      <label>Período académico<select value={filtros.ciclo} onChange={(e) => setFiltros({ ...filtros, ciclo: e.target.value })}><option value="">Todos</option>{periodos.map((x) => <option key={x.id} value={x.id}>{etiquetaPeriodo(x)}</option>)}</select></label>
