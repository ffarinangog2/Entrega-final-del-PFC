...
function humanMessage(status: number, backendMessage?: string) {
  ...
  if (status === 423) return backendMessage || 'La cuenta está bloqueada temporalmente.'
  if (status === 503) return 'El servicio no está disponible temporalmente. Intente nuevamente.'
