...
Existe una preparación para Firebase Cloud Messaging: la dependencia está
declarada, el servicio recibe mensajes y crea notificaciones, y
\texttt{onNewToken} reconoce una renovación del registro. Sin embargo, el plugin
de Google Services solo se aplica cuando existe \texttt{google-services.json},
archivo que no está versionado. Tampoco existe endpoint backend para registrar
el dispositivo ni un emisor que produzca mensajes. En consecuencia, FCM se
clasifica como preparado, pero no operativo de extremo a extremo. Como medida
de protección, el valor del token FCM no se escribe en los registros; solo se
informa que se recibió uno nuevo.
