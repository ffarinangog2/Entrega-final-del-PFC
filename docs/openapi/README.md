...
El Gateway usa `RouterFunction`, no controladores MVC. Su prueba forma la población desde los contratos de backend previamente contrastados con Spring runtime y envía cada operación a los routers reales con backends locales efímeros. Verifica tanto aceptación como backend y reescritura de path. La definición formal de completitud es: todas las operaciones runtime contractuales de backend que deben ser públicas son aceptadas por el `RouterFunction` correspondiente, más los alias explícitos realmente enrutados. El resultado es 182 operaciones canónicas, 8 alias Auth y 34 alias Usuarios: 224/224.
