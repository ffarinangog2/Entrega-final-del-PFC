...
public class ReservaServiceImpl implements ReservaService {
    ...
    public ReservaResponse cancelar(UUID id,CancelarReservaRequest request,UUID usuario){Reserva r=obtenerReservaParaActualizar(id);r.setEstado(ReservaStates.desde(r.getEstado()).cancelar());return reservaMapper.toResponse(reservaRepository.guardar(r));}
    @Override @Transactional(isolation=Isolation.SERIALIZABLE)
    ...
    @Override @Transactional(isolation=Isolation.SERIALIZABLE)
    public ReservaResponse finalizar(UUID id,UUID usuario){Reserva r=obtenerReservaParaActualizar(id);r.setEstado(ReservaStates.desde(r.getEstado()).finalizar());return reservaMapper.toResponse(reservaRepository.guardar(r));}
    @Override @Transactional(isolation=Isolation.SERIALIZABLE)
