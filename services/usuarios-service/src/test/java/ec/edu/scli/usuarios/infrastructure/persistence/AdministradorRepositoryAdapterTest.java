...
class AdministradorRepositoryAdapterTest {
    ...
    void setUp() {
        ...
        adapter = new AdministradorRepositoryAdapter(repository, mapper);
    ...
    private ec.edu.scli.usuarios.infrastructure.persistence.entity.Administrador entidad() {
    ...
    void save_deberiaMapearYPersistirUsandoReferenciaDePerfil() {
        ...
        Administrador guardado = adapter.save(dominio());

        assertThat(guardado.getId()).isEqualTo(administradorId);
        assertThat(guardado.getCodigoAdministrador()).isEqualTo("ADM-001");
    }

    @Test
    void findAll_conPaginacion_deberiaMapearPageResult() {
        Page<ec.edu.scli.usuarios.infrastructure.persistence.entity.Administrador> pagina =
        ...
        PageResult<Administrador> resultado =
    ...
    void findAll_sinArgumentos_deberiaMapearListaCompleta() {
        ...
        List<Administrador> resultado = adapter.findAll();

        assertThat(resultado).hasSize(1);
        assertThat(resultado.get(0).getId()).isEqualTo(administradorId);
    }

    @Test
    void findById_deberiaRetornarAdministrador_cuandoExiste() {
        when(repository.findById(administradorId)).thenReturn(Optional.of(entidad()));

        Optional<Administrador> resultado = adapter.findById(administradorId);

        assertThat(resultado).isPresent();
    }

    @Test
    void findByPerfilId_deberiaRetornarAdministrador_cuandoExiste() {
        when(repository.findByPerfilId(perfilId)).thenReturn(Optional.of(entidad()));

        Optional<Administrador> resultado = adapter.findByPerfilId(perfilId);
    ...
    @Test
    void existsByCodigoAdministrador_deberiaDelegarEnRepository() {
        when(repository.existsByCodigoAdministrador("ADM-001")).thenReturn(true);

        assertThat(adapter.existsByCodigoAdministrador("ADM-001")).isTrue();
    }
}
